...
- **Load Page** — in sidepanel mode uses `{ active: true, currentWindow: true }` to reliably get the tab in the current browser window; in popup mode queries all windows and filters out `chrome-extension://` URLs.
- **YouTube transcript extraction** uses a multi-step pipeline:
  ...
  4. *DOM scrape fallback* — if timedtext URLs return empty (YouTube's ASR captions require a Proof-of-Origin Token), the panel automation clicks the "Show transcript" / "顯示字幕" button (with a specific `ytd-video-description-transcript-section-renderer` selector or broader label search), polls for `ytd-transcript-segment-renderer` elements, and falls back to the engagement-panel's `innerText` if element selectors miss. If the panel is already open, the step is skipped to avoid toggling it off. If the button was clicked successfully, the page's `innerText` (now containing the transcript) is returned and marked `isYouTubeTranscript: true`.
